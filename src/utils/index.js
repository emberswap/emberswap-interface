...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://dex.emberswap.com/` +
      (remove ? `remove` : `add`) +
      `${token0Address === '0x3743ec0673453e5009310c727ba4eaf7b3a1cc04/' ? 'ETH/' : '/' + token0Address}/${'ETH'}`
    )
  } else {
    return (
      `https://dex.emberswap.com/` +
      (remove ? `remove` : `add`) +
      `${token0Address === '0x3743ec0673453e5009310c727ba4eaf7b3a1cc04/' ? 'ETH/' : '/' + token0Address}/${
        token1Address === '0x3743ec0673453e5009310c727ba4eaf7b3a1cc04/' ? 'ETH/' : '/' + token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://dex.emberswap.com/swap?inputCurrency=${token0Address}`
  } else {
    return `https://dex.emberswap.com/swap?inputCurrency=${
      token0Address === '0x3743ec0673453e5009310c727ba4eaf7b3a1cc04' ? 'ETH' : token0Address
    }&outputCurrency=${token1Address === '0x3743ec0673453e5009310c727ba4eaf7b3a1cc04' ? 'ETH' : token1Address}`
  }
}

export function getMiningPoolLink(token0Address) {
  return `https://dex.emberswap.com/ETH/${token0Address}`
}

export function getEmberswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://dex.emberswap.com'
  if (!linkVariable) {
