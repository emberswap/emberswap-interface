    ...
    let bulkResults = await Promise.all(
      current &&
        ...
        current?.data?.tokens.map(async (token) => {
          ...
          // HOTFIX for Aave
          //if (data.id === '0x7fc66500c84a76ad7e9c93437bfc5ac33e2ddae9') 0x6babf5277849265b6738e75aec43aefdde0ce88d{
          //  const aaveData = await client.query({
          //    query: PAIR_DATA('0x52c656faf57dcbdddd47bcba7b2ab79e4c232c28'),
          //    fetchPolicy: 'cache-first',
          //  })
...
export function useTokenPriceData(tokenAddress, timeWindow, interval = 3600) {
  ...
  useEffect(() => {
    ...
    const startTime =
      timeWindow === timeframeOptions.ALL_TIME ? 1645386025 : currentTime.subtract(1, windowSize).startOf('hour').unix()
