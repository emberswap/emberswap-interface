...
export default function Bridge() {
  ...
  const [chainTo, setChainTo] = useState<Chain | null>(
    chainId == ChainId.SMARTBCH ? DEFAULT_CHAIN_FROM : DEFAULT_CHAIN_TO
  )
  ...
  const { data: anyswapInfo, error }: SWRResponse<AnyswapTokensMap, Error> = useSWR(
    'https://bridgeapi.anyswap.exchange/v2/serverInfo/10000',
    (url) =>
      fetch(url)
    ...
    let tokens: Currency[] = Object.keys((anyswapInfo && anyswapInfo[chainFrom.id]) || {})
      ...
      .map((r) => {
        ...
        if (r.toLowerCase() == WNATIVE[chainFrom.id].address.toLowerCase()) {
          if (chainFrom.id == ChainId.SMARTBCH) {
            return BCH.onChain(chainFrom.id)
          }
  ...
  const handleChainFrom = useCallback(
    (chain: Chain) => {
      ...
      if (changeTo.id !== ChainId.SMARTBCH && chain.id !== ChainId.SMARTBCH) {
  ...
  const anyswapChains = [ChainId.SMARTBCH, ChainId.BSC, ChainId.MAINNET]
  ...
  return (
    <>
      ...
      <Head>
        <title>{i18n._(t`Bridge`)} | EmberSwap</title>
